...
#path1 is the address to the folder of all subjects, each subject has three txt files for alert, semisleepy and sleepy levels
path1='Drowsiness/RLDD+DROZY'
window_size=30
stride=2
Training='./Blinks_30_Fold5.npy'
Testing='./BlinksTest_30_Fold5.npy'
#################Normalizing with respect to different individuals####First Phase
blinks,labels,blinksTest,labelTest=Preprocess(path1,window_size,stride,test_fold='Fold5')
np.save(open(Training,'wb'),blinks)#path1 is the address to the folder of all subjects, each subject has three txt files for alert, semisleepy and sleepy levels
path1='Drowsiness/RLDD+DROZY'
window_size=30
stride=2
...
for i in range(1, 6):  # Assuming there are 5 folds
    fold = 'Fold' + str(i)
    Training='./Blinks_30_' + fold + '.npy'
    Testing='./BlinksTest_30_' + fold + '.npy'
    #################Normalizing with respect to different individuals####First Phase
    blinks,labels,blinksTest,labelTest=Preprocess(path1,window_size,stride,test_fold=fold)
    np.save(open(Training,'wb'),blinks)
    np.save(open('./Labels_30_' + fold + '.npy', 'wb'),labels)
    np.save(open(Testing, 'wb'),blinksTest)
    np.save(open('./LabelsTest_30_' + fold + '.npy', 'wb'),labelTest)
np.save(open('./Labels_30_Fold5.npy', 'wb'),labels)
np.save(open(Testing, 'wb'),blinksTest)
np.save(open('./LabelsTest_30_Fold5.npy', 'wb'),labelTest)