...
for i in range(5): #Cross validation but recommended to run each fold a few times to see the best perfomrance as you may
    ...
    #####
    BlinksTest[:,:,4]=(BlinksTest[:,:,4]-np.mean(Blinks[:,:,4]))/np.std(Blinks[:,:,4])
    Blinks[:,:,4]=(Blinks[:,:,4]-np.mean(Blinks[:,:,4]))/np.std(Blinks[:,:,4])
    ####
    ...
    if i==0:
        ...
        # start_indices=[0,55,138,163,218,326,336,362,363,373,400,446,448,509,555,618] # for step_size=30
        start_indices = [0, 56, 121, 170, 171, 179, 195, 203, 225, 262, 268, 344, 361, 371, 430, 474, 503, 554, 656,
                         698, 735, 765, 804, 933, 1001, 1003, 1011, 1134, 1157, 1193, 1242]
    ...
    if i==1:
        ...
        # start_indices=[0,40,140,199,239,398,401,405,406,409,414,479,507,530,605,656]  # for step_size=30
        start_indices = [0, 25, 32, 71, 99, 121, 178, 212, 253, 301, 302, 347, 378, 407, 418, 519, 588, 723, 932, 933,
                         934, 951, 956, 1002, 1065, 1135, 1224, 1260, 1334, 1443, 1526]
    ...
    if i==2:
        ...
        # start_indices=[0,75,113,213,288,426,501,611,696,771,966,1006,1131,1205,1245,1444] # for step_size=30
        start_indices = [0, 15, 122, 245, 284, 394, 499, 509, 536, 548, 549, 550, 551, 710, 900,
                         979, 980, 981, 989, 990, 1102, 1103, 1104, 1109, 1117, 1155, 1173, 1174, 1175, 1176, 1177]
    ...
    if i==3:
        ...
        # start_indices=[0,83,102,237,320,474,524,543,544,594,614,702,728,729,817,844]# for step_size=30
        start_indices = [0, 134, 235, 444, 476, 541, 636, 641, 718, 813, 814, 835, 944, 945, 999, 1073, 1109, 1203,
                         1355, 1380, 1439, 1490, 1504, 1532, 1606, 1610, 1611, 1646, 1649, 1807, 1920]
    ...
    if i==4:
        ...
        # start_indices=[0,198,286,287,485,634,674,675,774]# for step_size=30
        start_indices = [0, 3, 161, 274, 275, 300, 392, 393, 413, 425, 513, 545, 562, 594, 659, 754, 755, 766, 784, 839,
                         922, 947, 1012, 1040, 1063, 1084, 1192, 1281, 1292, 1329, 1348, 1349, 1410, 1440, 1480, 1546, 1655]
    ...
    x,loss,accuracy,loss_Test,accuracy_Test,acc_per_Vid=Train(total_input=Blinks,total_labels=Labels,TestB=BlinksTest,TestL=LabelsTest,
                    output_size=1,feature_size=5,batch_size=64,num_epochs=80,Pre_fc1_size=32,Post_fc1_size_per_layer=16,
