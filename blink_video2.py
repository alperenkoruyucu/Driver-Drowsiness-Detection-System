...
def blink_detector(output_textfile,input_video):
    ...
    while True:
        ...
        if (np.size(rects) != 0):
            ...
            TOTAL_MAR = TOTAL_MAR + MAR
            AVG_MAR = TOTAL_MAR / number_of_frames
            ...
            if MAR > MOUTH_AR_THRESH:
               MCOUNTER += 1
                ...
                if MCOUNTER > MCOUNTER_TEMP:
                    MCOUNTER_TEMP = MCOUNTER
                MCOUNTER = 0
                    ...
                    for detected_blink in retrieved_blinks:
                        ...
                        if(detected_blink.velocity>0):
                          with open(output_file, 'ab') as f_handle:
                             f_handle.write(b'\n')
                             np.savetxt(f_handle,[TOTAL_BLINKS,BLINK_FRAME_FREQ*100,detected_blink.amplitude,detected_blink.duration,detected_blink.velocity,AVG_MAR], delimiter=', ', newline=' ',fmt='%.4f')
                             MCOUNTER_TEMP = 0
...
if os.path.exists("Drowsiness/RLDD/Fold1/1/alert.txt"):
  os.remove("Drowsiness/RLDD/Fold1/1/alert.txt")
else:
  print("The file does not exist")

output_file = 'Drowsiness Data/Fold1/1/alert.txt'  # The text file to write to (for blinks)#
path = 'MaleGlasses.avi' # the path to the input video
...
# for x in range(1, 15):
#     for y in range (1, 4):
#         output_file = str(x) + '-' + str(y) + '.txt'
#         path = 'dataset/' + str(x) + '-' + str(y) + '.mp4'
#         print(path)
#         blink_detector(output_file, path)
#         print("")
