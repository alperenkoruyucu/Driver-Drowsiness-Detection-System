...
# this "adjust_gamma" function directly taken from :
                ...
                if ((reference_frame - 6)
                    ...
                    if Last_Blink.peakEAR < Last_Blink.startEAR and Last_Blink.peakEAR < Last_Blink.endEAR and Last_Blink.amplitude > MIN_AMPLITUDE and Last_Blink.start < Last_Blink.peak:
                        if ((Last_Blink.startEAR - Last_Blink.peakEAR) >
                            ...
                            (Last_Blink.endEAR - Last_Blink.peakEAR)
                            ):  # the amplitude is balanced
                            BLINK_READY = True
...
#output_file = '1alert.txt'  # The text file to write to (for blinks)#
#path = 'Datasets/RLDD/Fold1_part1/Fold1_part1/01/0.mov'  # the path to the input video
...
# Path listesi
paths = [
    'Datasets/Ben/Fold1_part1/Fold1_part1/02/10.mp4',




]
# Drowsiness klasörü oluşturma
drowsiness_folder = 'Drowsiness/NITYMED'
if not os.path.exists(drowsiness_folder):
    os.makedirs(drowsiness_folder)

# Loop
for path in paths:
    folder_names = path.split('/')
    fold_name = folder_names[-4]
    parent_folder_name = folder_names[-3]
    video_folder_name = folder_names[-2]
    video_name = os.path.splitext(os.path.basename(path))[0]

    if video_name.endswith('0'):
        output_file = os.path.join(drowsiness_folder, f'{fold_name}_{parent_folder_name}_{video_folder_name}_{video_name}_alert.txt')
    elif video_name.endswith('5'):
        output_file = os.path.join(drowsiness_folder, f'{fold_name}_{parent_folder_name}_{video_folder_name}_{video_name}_semisleepy.txt')
    elif video_name.endswith('10'):
        output_file = os.path.join(drowsiness_folder, f'{fold_name}_{parent_folder_name}_{video_folder_name}_{video_name}_sleepy.txt')
