...
def vote_accuracy_per_video(labels, predictions, starts_list, idx):
    ...
    for i, start in enumerate(starts_list):
        if (i + 1) == len(starts_list):
            ...
            Y = labels[start:starts_list[i + 1]]
        predicted_index = (predicts // 3.34).astype(np.int8)
        predicted_labels_temp = labels_pool[predicted_index]
        alert_voted_percent = np.sum(predicted_labels_temp == labels_pool[0]) / len(Y)
        lowVigilant_voted_percent = np.sum(predicted_labels_temp == labels_pool[1]) / len(Y)
        drowsy_voted_percent = np.sum(predicted_labels_temp == labels_pool[2]) / len(Y)
        final_decision = labels_pool[np.argmax([alert_voted_percent, lowVigilant_voted_percent, drowsy_voted_percent])]
        if len(Y) != 0:
            true_label = Y[0, 0]
            detected_label = final_decision
            true_labels = np.append(true_labels, true_label)
            predicted_labels = np.append(predicted_labels, detected_label)
            if final_decision == Y[0, 0]:
                count = count + 1
            if idx % 10 == 0:
                print(str(i + 1) + ': ' + 'True label is :%d and the detected label is %d' % (true_label, detected_label))
...
for i in range(5): #Cross validation but recommended to run each fold a few times to see the best perfomrance as you may
    ...
    if i==0:
        # start_indices = [0, 56, 121, 170, 171, 179, 195, 260, 288, 311, 325, 408, 419, 465, 467, 528, 538, 564, 565, 573,
        #                 595, 632, 638, 714, 731, 741, 800, 844, 873, 924, 1026, 1068, 1105, 1135, 1174, 1303, 1371, 1373,
        #                 1381, 1504, 1572, 1563, 1612] #for rldd+drozy+nthu

        start_indices = [0, 56, 121, 170, 171, 179, 195, 220, 227, 266, 300, 341, 389, 395, 419, 456, 462, 538, 555, 565,
                         624, 668, 697, 748, 850, 892, 929, 959, 1047, 1085, 1185, 1260, 1388, 1455, 1478, 1514, 1563]  # for rldd+drozy

        # start_indices = [0, 56, 121, 170, 171, 179, 195, 203, 225, 262, 268, 344, 361, 371, 430, 474, 503, 554, 656, 698,
        #                 735, 765, 804, 933, 1001, 1003, 1011, 1134, 1157, 1193, 1242] # for RLDD

        #start_indices = [0, 55, 138, 163, 173, 174, 200, 246, 248, 309] # for drozy

        #start_indices=[0,52,106,156,164,182,219,224,303,318,330,398,443,471,521,615,657,693,718,753,879,948,949,965,
        #               1068,1091,1126,1177,1178,1185,1200,1221,1226] # for step_size=30 RLDD
    ...
    if i==1:
        # start_indices = [0, 25, 32, 71, 99, 121, 178, 196, 222, 271, 278, 280, 288, 328, 428, 487, 542, 625, 650, 684,
        #                 725, 773, 819, 850, 879, 890, 991, 1060, 1195, 1404, 1405, 1406, 1423, 1428, 1474, 1537, 1607,
        #                 1696, 1732, 1806, 1915, 1998] # for rldd+drozy+nthu

        start_indices = [0, 18, 45, 57, 145, 146, 147, 306, 496, 575, 614, 743, 811, 813, 821, 944, 945, 946, 963, 968,
                         1014, 1077, 1147, 1236, 1272, 1273, 1318, 1349, 1378, 1389, 1490, 1559, 1694, 1903]  # for rldd+drozy

        # start_indices = [0, 25, 32, 71, 99, 121, 178, 212, 253, 301, 302, 347, 378, 407, 418, 519, 588, 723, 932, 933,
        #                934, 951, 956, 1002, 1065, 1135, 1224, 1260, 1334, 1443, 1526] # for RLDD

        #start_indices = [0, 40, 140, 199, 202, 206, 207, 272, 300, 323]  # for drozy

        #start_indices=[0,1,104,126,155,168,268,339,491,695,696,697,712,715,761,818,886,973,1012,1087,1189,1270,1298,
        #               1320,1380,1397,1406,1429,1439,1463,1464,1465,1549,1592,1753,1945]  # for step_size=30 RLDD
    ...
    if i==2:
        # start_indices = [0, 15, 122, 245, 284, 394, 499, 500, 600, 601, 602, 640, 643, 731, 769, 869, 879, 906, 921,
        #                 1080, 1270, 1349, 1350, 1351, 1359, 1360, 1472, 1473, 1474, 1479, 1487, 1525, 1543, 1544, 1545,
        #                 1546, 1547] #for rldd+drozy+nthu

        start_indices = [0, 46, 48, 109, 110, 222, 223, 224, 229, 237, 275, 293, 294, 334, 434, 493, 494, 553, 593, 667,
                         776, 859, 887, 909, 966, 981, 1088, 1211, 1294, 1419, 1493, 1691, 1779, 1914, 1915, 1916, 1924]  # for rldd+drozy

        # start_indices = [0,15, 122, 245, 284, 394, 499, 509, 536, 548, 549, 550, 551, 710, 900, 979, 980, 981, 989, 990,
        #                 1102, 1103, 1104, 1109, 1117, 1155, 1173, 1174, 1175, 1176, 1177] # FOR RLDD

        #start_indices = [0, 88, 126, 226, 301, 411, 496]  # for drozy

        #start_indices=[0,1,2,7,11,19,36,71,89,97,263,505,740,779,888,995,1129,1232,1435,1441,1496,1502,1503,1524,
        #               1617,1618,1662,1755,1790,1881,2039,2065,2138,2183,2184,2190] # for step_size=30 RLDD
    ...
    if i==3:
        # start_indices = [0, 134, 235, 444, 476, 541, 636, 748, 752, 753, 812, 852, 902, 921, 922, 927, 1004, 1005, 1006,
        #                 1027, 1136, 1137, 1191, 1265, 1301, 1395, 1547, 1572, 1631, 1682, 1696, 1724, 1798, 1802, 1803,
        #                 1838, 1841, 1999, 2112] #for rldd+drozy+nthu

        start_indices = [0, 1, 2, 3, 53, 72, 73, 87, 115, 189, 191, 194, 229, 268,  378, 483, 617, 718, 927, 932, 987,
                         990, 991, 1012, 1121, 1122, 1176, 1250, 1286, 1380, 1532, 1557, 1616, 1667, 1677, 1703, 1704]  # for rldd+drozy

        # start_indices = [0, 134, 235, 444, 476, 541, 636, 641, 718, 813, 814, 835, 944, 945, 999, 1073, 1109, 1203, 1355,
        #                 1380, 1439, 1490, 1504, 1532, 1606, 1610, 1611, 1646, 1649, 1807, 1920] #for rldd

        #start_indices = [0, 83, 208, 282, 332, 351, 352]  # for drozy

        #start_indices=[0,8,38,108,113,114,135,137,285,380,411,473,568,569,581,590,610,699,819,827,865,884,885,923,949,990,
        #               1051,1156,1157,1185,1265,1266,1284,1299,1391,1450]# for step_size=30 RLDD
    ...
    if i==4:
        # start_indices = [0, 1, 12, 30, 31, 50, 109, 307, 395, 530, 613, 738, 812, 887, 997, 1082, 1103, 1211, 1300, 1311,
        #                 1348, 1367, 1368, 1429, 1459, 1499, 1565, 1674, 1700, 1792, 1793, 1813, 1825, 1913, 1945, 1962,
        #                 1965, 1969, 1970] #for rldd+drozy+nthu

        start_indices = [0, 3, 161, 274, 275, 300, 392, 393, 413, 425, 513, 545, 562, 594, 659, 754, 755, 766, 784, 839,
                         922, 947, 1012, 1040, 1063, 1084, 1192, 1281, 1292, 1329, 1348, 1349, 1410, 1440, 1480, 1546, 1655]  # for rldd+drozy

        #start_indices = [0, 21, 59, 83]  # for Ben

        # start_indices = [0, 1, 12, 30, 51, 159, 248, 259, 296, 315, 316, 377, 407, 447, 513, 622, 623, 648, 740, 741, 761,
        #                 773, 861, 893, 910] # for RLDD

        #start_indices = [0, 198, 286, 421, 422, 481, 521]  # for drozy

        #start_indices=[0,1,27,45,55,153,205,256,382,515,521,522,527,536,556,575,598,730,794,795,904,955,999,
        #               1027,1059,1060,1088,1143,1275,1480,1605,1622,1659]# for step_size=30 RLDD
